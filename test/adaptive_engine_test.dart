...
void main() {
  group('Aufgabenpool', () {
    final facts = AdaptiveEngine.buildFactPool();

    test('alle Ergebnisse bleiben im Zahlenraum 0 bis 10', () {
      expect(facts, isNotEmpty);
      expect(facts.every((f) => f.result >= 0 && f.result <= 10), isTrue);
    });
    ...
    test('Plus erzeugt niemals Ergebnisse über 10', () {
      final plus = facts.where((f) => f.operation == MathOperation.plus);
      expect(plus.every((f) => f.a + f.b <= 10), isTrue);
    });
  });
  ...
  test('Unsichere Aufgabe wird stärker gewichtet ausgewählt', () {
    ...
    for (var i = 0; i < 7; i++) {
      strong.registerAttempt(correct: true, responseTime: const Duration(seconds: 2), usedHelp: false);
    }
    for (var i = 0; i < 4; i++) {
      weak.registerAttempt(correct: false, responseTime: const Duration(seconds: 8), usedHelp: true);
    }

    var weakCount = 0;
    var strongCount = 0;
    for (var i = 0; i < 400; i++) {
      final selected = engine.selectNext(facts: [weak, strong], mode: TrainingMode.minus);
  ...
  test('Progression beginnt mit einfachen Rechenbeziehungen', () {
    final engine = AdaptiveEngine(random: Random(3));
    final facts = AdaptiveEngine.buildFactPool();
    expect(engine.progressionTier(facts), 1);
    for (var i = 0; i < 40; i++) {
      final fact = engine.selectNext(facts: facts, mode: TrainingMode.practice);
